...
export default class TransactionsSplit extends BaseCommand {
  static override args = {
    id: Args.integer({
      description:
        'Unique identifier of the transaction to split (integer). Cannot be a recurring, grouped, or already-split transaction.',
      required: true,
    }),
  }
  ...
  static override flags = {
    parts: Flags.string({
      description:
        'JSON array of child transaction objects (2-500). Each requires \'amount\' (numeric, must sum to parent amount). Optional fields per child: payee (max 140 chars, inherits from parent), date (YYYY-MM-DD, inherits from parent), category_id (integer, inherits from parent), tag_ids (integer array), notes (max 350 chars, inherits from parent). Example: \'[{"amount":25.00,"payee":"Split 1"},{"amount":17.45,"payee":"Split 2"}]\'',
      required: true,
    }),
  }
