  ...
  it("maps boolean include flags", async () => {
    const { client } = await runCommand(
      ...
      [
        ...
        "--include-exclude-from-budgets", "--include-occurrences",
        "--include-past-budget-dates", "--include-totals", "--include-rollover-pool",
        "--json",
      ],
    );
    const params = client.summary.get.firstCall.args[0];
    ...
    const { stdout } = await runCommand(
      ...
      (c) => {
        c.summary.get.resolves({
          categories: [{ category_id: 42, totals: { budgeted: "500.00", recurring_activity: "100.00", other_activity: "200.00", available: "200.00" } }],
        });
      },
    );
